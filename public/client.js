$(function() {
    ...
    // 1      --  Change Page Title Once Time (when the webpage state is not visible)
    // 2      --  Flash Page Title (when the webpage state is not visible)
    // 3      --  Change Page Title Once Time (always, just notify in 3 seconds)
...
>>>>>>> Stashed changes
    ...
    $(document).on('click', '.socketchatbox-socketdetail-each', function() {
        ...
        if (user.id in selectedUsers) {
            ...
            if (socketID in selectedSockets) {
                user.selectedSocketCount--;
